...
class IosAssetGenerator extends asset_generator_1.AssetGenerator {
    ...
    async generate(asset, project) {
        ...
        switch (asset.kind) {
            ...
            case "notification-icon" /* NotificationIcon */:
                return this.generateNotificationIcons(asset, project);
            case "icon" /* Icon */:
                return [];
            case "settings-icon" /* SettingsIcon */:
                return this.generateSettingsIcons(asset, project);
            case "spotlight-icon" /* SpotlightIcon */:
    ...
    async generateIconsForLogo(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => ["icon" /* Icon */, "notification-icon" /* NotificationIcon */, "settings-icon" /* SettingsIcon */, "spotlight-icon" /* SpotlightIcon */].find((i) => i === a.kind));
    ...
    async generateNotificationIcons(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => a.kind === "notification-icon" /* NotificationIcon */);
        return this._generateIcons(asset, project, icons);
    }
    async generateSettingsIcons(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => a.kind === "settings-icon" /* SettingsIcon */);
        return this._generateIcons(asset, project, icons);
    }
    async generateSpotlightIcons(asset, project) {
        const icons = Object.values(IosAssetTemplates).filter((a) => a.kind === "spotlight-icon" /* SpotlightIcon */);
        return this._generateIcons(asset, project, icons);
    }
    ...
    async updateIconsContentsJson(generated, project) {
        ...
        for (const g of generated) {
            const width = g.template.width / ((_a = g.template.scale) !== null && _a !== void 0 ? _a : 1);
            const height = g.template.height / ((_b = g.template.scale) !== null && _b !== void 0 ? _b : 1);
            const scale = (_c = g.template.scale) !== null && _c !== void 0 ? _c : 1;
            const existing = withoutMissing.find((f) => f.scale === `${scale}x` &&
